...
parser.add_argument('--num_point', type=int, default=8192, help='Number of sampled points [default: 8192]')
parser.add_argument('--vox_size', type=float, default=1.5, help='Size of voxel [default: 1.5]')
...
parser.add_argument('--data', type=str, default='kitti_object_deeplab_reduced', help='Base name of data files')
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        start_idx = batch_idx * BATCH_SIZE
        end_idx = (batch_idx+1) * BATCH_SIZE
        batch_data, batch_label, batch_smpw = get_batch(VAL_DATASET, test_idxs, start_idx, end_idx)
...
def eval_whole_scene_one_epoch(sess, ops, test_writer):
    ...
    global EPOCH_CNT
    is_training = False
    test_idxs = np.arange(0, len(VAL_DATASET_WHOLE_SCENE))
    num_batches = len(VAL_DATASET_WHOLE_SCENE)
    ...
    for batch_idx in range(num_batches):
        if not is_continue_batch:
            batch_data, batch_label, batch_smpw = VAL_DATASET_WHOLE_SCENE[batch_idx]
            ...
            batch_data_tmp, batch_label_tmp, batch_smpw_tmp = VAL_DATASET_WHOLE_SCENE[batch_idx]
